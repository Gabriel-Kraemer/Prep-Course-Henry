...
function dePalabrasAFrase(palabras) {
  ...
  // Tu código:
  let palabraconca = palabras[0];
  for (var i = 1; i < palabras.length; i++){
    palabraconca = palabraconca + ' ' + palabras[i] ;
  }
  return palabraconca;
}
...
function arrayContiene(array, elemento) {
  ...
  // Tu código:
  if (array.find(x => x === elemento)) return true;
  return false;
}
...
function continueStatement(numero) {
  ...
  let nuevo = [];
  for (var i = 1; i < 11; i++){
    if (i === 5) continue; // "Se interrumpió la ejecución";
    else {
      numero = numero + 2;
      nuevo.push(numero);
    }
